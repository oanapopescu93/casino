...
function blackjack_wheel(props){
	...
	let button_hit = {}
	let button_stay = {}
	let start_button_coordonates = {};
	let clear_button_coordonates = {};
	let hit_button_coordonates = {};
	let stay_button_coordonates = {};
	...
	this.draw_table_spots = function(){
		let my_width = canvas.width;
		let space_after_dealer = 50;
		if (window.innerWidth < 960){
			space_after_dealer = 35;
		}
		let space = (my_width - (card_base.width*7 + card_base.x*6))/2;
		let a = 0;
		let b = 0;
		for(let i=0; i<7; i++){
	...
	this.draw_card_values = function(fillStyle, lineWidth, strokeStyle, color){
		for(let i in all_cards_value){				
			let value_hand = all_cards_value[i].value_hand;
			ctx.beginPath();
	...
	this.draw_card = function(x, y, w, h, hand, elem, id){
		let img = document.getElementById("img_cards");
		let img_x = 0;
		let img_y = 0;
		let a = 75;
		let b = 120;
		...
		if(hand === "hidden"){
			...
			for(let i in hand){		
				...
				//};				
				let card_pos = {elem: elem, id: parseInt(id), x: x + i*12, y: y + i*12 + space, card: hand[i]}
				all_cards_pos.push(card_pos);
				value_hand = value_hand + hand[i].Weight;
			}
		}
		
		let card_value = {elem: elem, id: parseInt(id), x: x, y: y, card: hand, value_hand: value_hand}
		...
		if(isInside(mousePos, start_button_coordonates) || isInside(mousePos, clear_button_coordonates) || isInside(mousePos, hit_button_coordonates) || isInside(mousePos, stay_button_coordonates)){
			...
			for(let i in blackjack_pos){
				if(isInside(mousePos, blackjack_pos[i])){
					...
					console.log(user_join, user_id, pos)
...
function Blackjack(props) {	
	setTimeout(function(){ 	
		...
		let blackjack_game = new blackjack_wheel(props);
