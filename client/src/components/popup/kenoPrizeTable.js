        ...
        {kenoPrizes && kenoPrizes.length>0 ? <div className="keno_rules">
            {kenoPrizes.map((items, i)=>{ 
                ...
                return <div key={i} className="keno_rules_box">
                    <div className="keno_rules_box_header">{translate({lang, info: "keno_numbers_played"})}: {x}</div>
                    <div className="keno_rules_box_body">
                        <div className="keno_rules_items">
                            <div className="keno_rules_item">{translate({lang, info: "keno_numbers_matched"})}</div>
                            <div className="keno_rules_item">{translate({lang, info: "keno_win"})}</div>
                        </div>
                        {items.map((item, j)=>{ 
