...
io.on('connection', function(socket){
	...
	socket.on('change_username_send', function(data) {
		if(data.uuid){
			...
			if(user_found){
				database_config.sql = "UPDATE casino_user SET user='" + user_new + "' WHERE id=" + user_found.id + '; '
				database_config.sql += "SELECT * FROM casino_user"
	...
	socket.on('change_password_send', function(data) {
		if(data.uuid){
			...
			if(user_found){
				database_config.sql = "UPDATE casino_user SET pass='" + new_pass_encrypt + "' WHERE id=" + user_found.id + '; '
				database_config.sql += "SELECT * FROM casino_user"
				database(database_config).then(function(){
					try{
	...
	socket.on('change_pic_send', function(data) {
		if(data.uuid){
			...
			if(user_found){
				database_config.sql = "UPDATE casino_user SET profile_pic='" + pic + "' WHERE id=" + user_found.id
				database(database_config).then(function(){
					try{
